  ...
  class RepositoriesControllerTest < ActionDispatch::IntegrationTest
    ...
    test 'should show repository in json format' do
      VCR.use_cassette('github/repositories_controller') do
        ...
        expected_keys = %w[id node_id name full_name private owner html_url description fork url forks_url keys_url
                           ...
                           has_wiki has_pages forks_count mirror_url archived disabled open_issues_count license
                           allow_forking is_template topics visibility forks open_issues watchers default_branch
                           permissions temp_clone_token allow_squash_merge allow_merge_commit allow_rebase_merge
                           allow_auto_merge delete_branch_on_merge allow_update_branch organization network_count
                           subscribers_count]
        assert(expected_keys.all? { |k| json_response['repository'].key? k })
        assert_equal @app.github_repo, json_response['repository']['name']
      end
    end
  end
end
