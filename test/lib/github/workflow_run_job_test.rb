  ...
  class WorkflowRunJobTest < ActiveSupport::TestCase
    setup do
      VCR.use_cassette('github/workflow_run_job') do
        @workflow_run_job = Github::WorkflowRunJob.new('department-of-veterans-affairs/vets-api', 5_169_530_176)
      end
    end
    ...
    test 'get logs returns check-run results when logs do not exist' do
      VCR.use_cassette('github/workflow_run_job', record: :new_episodes) do
        workflow_run_job = Github::WorkflowRunJob.new('department-of-veterans-affairs/vets-api', 5_204_164_825)
        logs = workflow_run_job.logs
        assert_kind_of String, logs
      end
    end
    ...
    test 'lists all workflow run jobs for a workflow run' do
      VCR.use_cassette('github/workflow_run_job', record: :new_episodes) do
        all_workflow_run_jobs = Github::WorkflowRunJob.all_for_workflow_run('department-of-veterans-affairs/vets-api', 1_834_786_549).jobs
