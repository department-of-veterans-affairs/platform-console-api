  ...
  class WorkflowTest < ActiveSupport::TestCase
    setup do
      VCR.use_cassette('github/workflow') do
        @workflow = Github::Workflow.new(ENV['GITHUB_ACCESS_TOKEN'],
    ...
    test 'lists all workflows for the given repository' do
      VCR.use_cassette('github/workflow', record: :new_episodes) do
        workflows = Github::Workflow.all(ENV['GITHUB_ACCESS_TOKEN'],
                                         'department-of-veterans-affairs/vets-api').workflows
    ...
    test 'dispatch a workflow' do
      VCR.use_cassette('github/workflow', record: :new_episodes) do
        dispatch = Github::Workflow.dispatch!(ENV['GITHUB_ACCESS_TOKEN'],
