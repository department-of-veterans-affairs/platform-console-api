    ...
    def rerun # rubocop:disable Metrics/AbcSize
      respond_to do |format|
        if @github_deploy_run.rerun!
          path = team_app_github_repository_deploy_path(@team, @app, @app.github_repo, params[:deploy_id])
          format.html { redirect_to path, notice: 'deploy run was sucessfully restarted' }
          format.json { render json: true }
        else
          path = team_app_github_repository_deploy_deploy_run_path(@team, @app, @app.github_repo,
